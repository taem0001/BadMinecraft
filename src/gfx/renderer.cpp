...
namespace Minecraft {
	namespace GFX {
		Renderer::Renderer()
			: cam(Camera(glm::vec3(0.0f, 70.0f, 0.0f))), streamer(ChunkStreamer()), width(WIDTH), height(HEIGHT) {
		...
		void Renderer::updateChunks(World::World &w) {
			World::ChunkCoord playerChunk = {floorDiv(cam.pos[0], CHUNK_MAX_X), floorDiv(cam.pos[2], CHUNK_MAX_Z)};
			streamer.updateChunks(w, playerChunk);
		}
		...
		void Renderer::renderWorld() {
			...
			const std::unordered_map<World::ChunkCoord, ChunkMesh> &meshes = streamer.getMeshes();
			...
			for (auto &[coord, mesh] : meshes) {
				...
				if (dist > streamer.RENDER_RADIUS_SQ) continue;

				glm::mat4 model = glm::translate(glm::mat4(1.0f), coord.worldOrigin());
		...
		Frustum Renderer::getFrustum() {
			Frustum f;
			const glm::mat4 VP = cam.getProjMat(width, height) * cam.getViewMat();
		...
		aabb Renderer::getAABB(const World::ChunkCoord &coord) {
			...
			glm::vec3 mx = mn + glm::vec3(CHUNK_MAX_X, CHUNK_MAX_Y, CHUNK_MAX_Z);
			return {mn, mx};
		}
