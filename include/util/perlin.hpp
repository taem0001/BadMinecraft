...
namespace Perlin {
	...
	double static constexpr lerp(double a, double b, double t) {
		return a + t * (b - a);
	}
	...
	double static constexpr perlin(double x, double y, double z) {
		...
		double x2 = lerp(grad(p[AB], xf, yf - 1, zf),
						 grad(p[BB], xf - 1, yf - 1, zf), u);
		double y1 = lerp(x1, x2, v);

		double x3 = lerp(grad(p[AA + 1], xf, yf, zf - 1),
						 grad(p[BA + 1], xf - 1, yf, zf - 1), u);
		double x4 = lerp(grad(p[AB + 1], xf, yf - 1, zf - 1),
